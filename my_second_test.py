...
def plot_figure1(spi_prob, double_prob_dict):
    ...
    for i, delta_u in enumerate(DELTA_U_VALUES):
        plt.plot(SNR_RANGE, double_prob_dict[delta_u], colors[i], 
                label=f'Seq. Sum [14], Δu={delta_u}', lw=2, markersize=6)
...
if __name__ == "__main__":
    ...
    print(f"SPI-ZC: {spi_prob[idx_0db]:.3f}")
    for delta_u in DELTA_U_VALUES:
        print(f"Double-ZC (Δu={delta_u}): {double_prob_dict[delta_u][idx_0db]:.3f}")
        print(f"  SPI-ZC相对提升: {spi_prob[idx_0db] - double_prob_dict[delta_u][idx_0db]:.3f}")
    ...
    idx_0db = np.where(SNR_RANGE == 0)[0][0]
    for method_name, method_results in results.items():
        print(f"{method_name}: {method_results[idx_0db]:.3f}")
