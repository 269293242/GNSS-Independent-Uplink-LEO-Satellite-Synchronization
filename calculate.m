%% N = 5: keep only maximum-PSR cases, then merge by shift-and-scale equivalence
N = 5;
...
Pmax = paramList(idxMax, :);
Ymax = Y(idxMax);

fprintf('N=%d: max PSR = %.12f, number of max-PSR parameter sets = %d\n', ...
    N, maxPSR, size(Pmax, 1));

% Group by circular shift + complex scaling
assigned = false(numel(Ymax), 1);
groups = {};
...
% Print only the number of unique groups
fprintf('%d\n', numel(groups));

% fprintf('Number of equivalence classes = %d\n\n', numel(groups));
% for g = 1:numel(groups)
%   rows = groups{g}(:);
%   fprintf('Group %2d (size %d):\n', g, numel(rows));
%   disp([rows, Pmax(rows,:)])   % columns: [index u0 ga gb u1 gc gd]
% end
